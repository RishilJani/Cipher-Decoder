...
class EncodeDecodeOptionsController extends GetxController{
  ...
  RxString desc  = ''.obs;
  RxList<EncodeDecodeMethods> options = <EncodeDecodeMethods>[CeaseCipher()].obs;

  void addWidget({ required EncodeDecodeMethods methodObj ,required controller }){
    if(options.length < maxLimit){
      ...
      Get.snackbar("Error","Can't add more methods",duration: const Duration(seconds: 5));
    }
  }
  ...
  void keyUpdateWidget({required index, controller }){
    ...
    if(options[index].requiresKey){
      options[index].key = controller.keyController.text.isNotEmpty  ? int.parse(controller.keyController.text)  : null;
    }
    if(controller is EncodingController){
...
Widget getOptionList({ controller }){
  ...
  return Obx(() => ListView.builder(
    ...
    itemBuilder: (context, index) {
      return EncodeDecodeOptions(controller: controller, index: index,);
    },
  ),);
}
